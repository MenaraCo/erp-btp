...
export default function StructurePage() {
  ...
  return (
    <div>
      ...
      <h1 style={{ marginBottom: 4 }}>Étude d'exécution</h1>
      <p className="muted" style={{ marginTop: 0, maxWidth: 760 }}>
        L'étude technique du chantier, marché par marché : ouvrages, ressources, quantités et prix objectif.
        Elle passe par trois phases — <strong>budget d'étude</strong> (repris du devis, à valider) →
        <strong> contre-étude</strong> (renégocier ratios, quantités, PU, prestations) → <strong>exécution</strong> —
        chaque validation étant horodatée. Elle CALCULE le budget ; l'enveloppe et ses ripages se pilotent dans
        <strong> Budgets</strong>, et ce qui est réellement fait se constate dans <strong>Avancement constaté</strong>.
      </p>
...
function MarcheBlock({ marche, chantierId, token }: { marche: MarcheTree; chantierId: string; token: string | null }) {
  ...
  const invalidate = () => {
    ...
    // L'étude CALCULE le budget : toute quantité ou tout PU renégocié déplace l'enveloppe.
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      ...
      {marche.execution_phase === 'execution' && (
        <p className="muted" style={{ fontSize: 12, marginTop: 6, marginBottom: 0 }}>
          Marché en exécution : l'étude est arrêtée. L'avancement se constate dans l'écran
          <strong> Avancement constaté</strong>, les mouvements d'enveloppe dans <strong>Budgets</strong>.
        </p>
      )}
...
function LineRows({ node, depth, edit }: { node: TreeNode; depth: number; edit: Edit }) {
  ...
  const cols = 5 + (edit.editable ? 1 : 0);
...
function ComponentRow({ comp, pad, edit }: { comp: CompNode; pad: number; edit: Edit }) {
  const trailing = edit.editable ? 1 : 0;
