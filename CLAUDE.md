...
- **Jetons = réserve commune** : un utilisateur n'accède à un module que si un **jeton lui est affecté sur ce module**. Les jetons du palier forment un **pool partagé au niveau de la société** : `pool = sièges achetés × nombre de modules du palier (Socle compris)` — « Pro » (Socle + Études de prix + Facturation) vaut 3 jetons par siège, « Pro Max » en vaut 5. Chaque couple **utilisateur × module** consomme **un** jeton, quel que soit le module : un jeton posé sur Études de prix diminue d'autant ce qui reste pour le Suivi de chantiers. Le client répartit donc librement (3 personnes sur un module chacune, ou 1 personne sur trois modules) à valeur vendue identique. Les **options (add-ons)**, achetées séparément au siège, gardent **leur propre compteur**, hors du pool. Jetons affectés ≤ jetons disponibles, contrôlé à l'affectation.
...
10. **Branchement complet — aucune fonction orpheline.** À chaque ajout ou modification d'une fonction, **vérifier et réaliser son branchement dans TOUTE l'application** : route/API exposée, **lien dans le menu/navigation**, **bouton ou écran qui l'appelle**, et **tous les endroits qui en dépendent** (autres modules, écrans, tableaux de bord, le moteur de contrôle de gestion si un coût/donnée est concerné). Une fonction qui existe mais n'est reliée à rien (« orpheline ») est considérée comme **non terminée**. Après chaque modification, faire un **balayage des points d'usage** et lister explicitement où la fonction a été branchée, et confirmer qu'aucun appelant attendu n'a été oublié. Si une modification change un comportement existant (signature, champ, règle), **mettre à jour tous les appelants** concernés, pas seulement l'endroit modifié.

---

## Définition de « terminé » pour une tâche

Une tâche n'est terminée que si : le code fonctionne **et** les tests passent **et** la migration de schéma est écrite (si le schéma a changé) **et** l'API est documentée (OpenAPI) **et** la fonction est **branchée partout où elle doit l'être** (menu, écran appelant, modules dépendants — aucune fonction orpheline) **et** un message de commit est proposé.
...
- **Phase 1 — Module Études de prix** (le cœur qui vend le produit) : bibliothèques + ressources, **ouvrages composés avec recalcul ascendant**, corps de devis hiérarchique, métré, déboursé/sous-détails, feuille de vente + coefficients, workflow d'affaire, édition PDF, devis d'appel d'offre. **Module Paramètres société** (en cours) : onglet Entreprise, Familles/Lots/Codes analytiques/Unités CRUD, Préférences (FG/bénéfice par défaut, taux TVA paramétrables, onglet devis par défaut, nb décimales, 2 couleurs). **Fiche ressource enrichie** (faite) : `apps/web/components/ResourceModal.tsx` — Famille (filtre en cascade les codes), Code analytique, Unité d'emploi et Unité d'achat, Distributeur et sa référence, Type de déboursé, tous tirés des référentiels ; coefficient de conversion avec réciproque affichée, et PU Débours auto-calculé = PU Public ÷ coeff (restant modifiable à la main).
...
- **Palette paramétrable** : fond clair, gris neutres pour les surfaces. **Deux couleurs configurables par société** dans Paramètres → Préférences : `--primary` (couleur principale, nav/titres, navy par défaut `#1a3a5c`) et `--accent` (couleur d'action, boutons/codes/badges, orange par défaut `#e8550a`). Appliquées en CSS vars dynamiques via `PrefsProvider`. Réserver le rouge aux alertes et dérives (contrôle de gestion).
